...
// DemoLogin sets a read-only demo session without touching the database.
// Used when DEMO_MODE is enabled in the frontend for UI review builds.
func DemoLogin() Response {
	demo := &models.User{
		ID:       0,
		Username: "demo",
		FullName: "Demo Viewer",
		Role:     "Admin", // Admin so all handlers pass IsLoggedIn/IsAdmin checks
		IsActive: true,
	}
	middleware.Store.Login(demo)
	return okResponse("Demo login", userToResponse(demo))
}
