...
Modules.Accounting = {
  async load(sub, id) {
    const active = sub || 'coa';
    showLoading();

    const tabBar = `
      <ul class="nav nav-tabs mb-4">
        <li class="nav-item">
          <a class="nav-link ${active === 'coa' ? 'active' : ''}" href="#"
            onclick="navigate('#/accounting/coa');return false;">
    ...
    if (active === 'coa') await loadChartOfAccounts(tabBar);
    ...
    else await loadChartOfAccounts(tabBar);
  }
};
...
async function loadChartOfAccounts(tabBar) {
  const accounts = await api.GetChartOfAccounts();
  _coaList = accounts || [];
      ...
      .map(a => {
        ...
        return `
          <tr>
            ...
            <td class="text-end">
              <button class="btn btn-sm btn-outline-secondary" onclick="openCoaModal(${a.id})">
      ...
      <div class="modal-dialog">
        <div class="modal-content border-0 shadow">
          <div class="modal-header">
            <h5 class="modal-title" id="coaModalTitle">Account</h5>
          ...
          <div class="modal-body">
            ...
            <div class="row g-3">
              <div class="col-md-6">
                ...
                <input type="text" class="form-control" id="coaCode" required placeholder="e.g. 1000">
              </div>
              <div class="col-md-6">
                <label class="form-label">Account Name <span class="text-danger">*</span></label>
                <input type="text" class="form-control" id="coaName" required>
              </div>
              <div class="col-md-6">
                <label class="form-label">Account Type</label>
                <select class="form-select" id="coaType">
              ...
              <div class="col-md-6">
                ...
                <select class="form-select" id="coaNormalBalance">
                  <option value="">Select…</option>
              ...
              <div class="col-12">
                ...
                <select class="form-select" id="coaParent">
                  <option value="">— None (top level) —</option>
                  ${_coaList.map(a => `<option value="${a.id}">${a.code} — ${a.name}</option>`).join('')}
                </select>
              </div>
            </div>
          </div>
          <div class="modal-footer">
            ...
            <button class="btn btn-primary" onclick="submitCoaForm()">Save</button>
          </div>
        </div>
      </div>
    </div>
...
function openCoaModal(id) {
  ...
  document.getElementById('coaModalTitle').textContent = isEdit ? 'Edit Account' : 'Add Account';
  document.getElementById('coaId').value = id || '';
  if (isEdit) {
    const a = _coaList.find(x => x.id === id);
    if (a) {
      document.getElementById('coaCode').value = a.code || '';
      document.getElementById('coaName').value = a.name || '';
      document.getElementById('coaType').value = a.account_type || '';
      document.getElementById('coaNormalBalance').value = a.normal_balance || '';
      document.getElementById('coaParent').value = a.parent_id ?? '';
    }
  } else {
    ['coaCode','coaName'].forEach(id => document.getElementById(id).value = '');
    document.getElementById('coaType').value = '';
    document.getElementById('coaNormalBalance').value = '';
    document.getElementById('coaParent').value = '';
  }
  new bootstrap.Modal(document.getElementById('coaModal')).show();
}

async function submitCoaForm() {
  const id = document.getElementById('coaId').value;
  const parentVal = document.getElementById('coaParent').value;
  const payload = {
    ...
    normal_balance: document.getElementById('coaNormalBalance').value,
...
async function loadAccountDetermination(tabBar) {
  ...
  const postingAccounts = _coaList.filter(a => a.account_type === 'POSTING' && a.is_active);
  ...
  const rows = entries.length === 0
    ...
    : entries.map(e => `
        <tr style="cursor:pointer" onclick="navigate('#/accounting/journal-entries/${e.id}')">
          ...
          <td>${e.narration || '—'}</td>
          <td><span class="badge bg-${e.status === 'Posted' ? 'success' : 'warning'}">${e.status || 'Draft'}</span></td>
...
async function loadJournalEntryDetail(id, tabBar) {
  ...
  showView(wrapAccounting(tabBar, `
    <div class="d-flex align-items-center gap-2 mb-4">
      ...
      <span class="badge bg-${je.status === 'Posted' ? 'success' : 'warning'} ms-2">${je.status || 'Draft'}</span>
    </div>
    <div class="row g-3 mb-4">
      <div class="col-sm-4"><strong>Date:</strong> ${formatDate(je.entry_date)}</div>
...
async function loadNewJournalEntry(tabBar) {
  ...
  window._jeAccountOptions = accountOptions;
...
async function submitJe() {
  ...
  document.querySelectorAll('#jeLines tr').forEach(tr => {
    ...
    if (accId && (dr > 0 || cr > 0)) {
      lines.push({ gl_account_id: parseInt(accId, 10), debit: dr, credit: cr, remarks: rem });
      totalDr += dr;
      totalCr += cr;
    }
  });
  ...
  const payload = { entry_date: entryDate, narration, lines };
