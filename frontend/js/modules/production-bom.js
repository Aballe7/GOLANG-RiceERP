...
async function loadBOMList(tabBar) {
  ...
  const rows = boms.length === 0
    ? `<tr><td colspan="4" class="text-center text-muted py-4">No BOMs yet. Create one to enable Work Orders.</td></tr>`
    : boms.map(b => `
        <tr>
          <td class="font-monospace fw-semibold small">${b.code}</td>
          <td class="text-muted small">${formatNumber(b.quantity, 3)}</td>
          <td>${(b.lines || []).length} component${(b.lines || []).length !== 1 ? 's' : ''}</td>
          <td class="text-end">
        ...
        <div class="modal-content">
          ...
          <div class="modal-body">
            <div class="row g-3 mb-4">
              <div class="col-md-4">
                <label class="form-label fw-bold">Finished Item Code <span class="text-danger">*</span></label>
                <input type="text" id="bomCode" class="form-control font-monospace" placeholder="e.g. MILLED-RICE-STD">
                <div class="form-text">Must match an Item Master code with mak_item = Yes</div>
              </div>
              <div class="col-md-2">
            ...
            <div class="d-flex justify-content-between align-items-center mb-2">
              <div class="fw-bold small"><i class="bi bi-list-ul me-1 text-primary"></i>Components</div>
              ...
              <table class="table table-sm align-middle" id="bomLinesTable">
                <thead class="table-light">
                  <tr>
                    ...
                    <th style="width:90px">Issue Method</th>
...
window.saveBOM = async function() {
  ...
  document.querySelectorAll('#bomLinesBody tr').forEach(tr => {
    ...
    if (itemCode && qty > 0) {
      lines.push({ item_code: itemCode, quantity: qty, warehouse: whs, issue_method: method });
    }
  });
